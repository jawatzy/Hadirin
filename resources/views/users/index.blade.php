...
<head>
  ...
  <title>User Management - Hadirin</title>
  <style>
    @keyframes fadeIn {
...
<body class="bg-gray-50 text-gray-800 min-h-screen">
  ...
  <div class="w-full px-4 md:px-6 pt-8 pb-6 bg-white rounded-t-3xl -mt-4 min-h-[65vh] max-w-6xl mx-auto fade-in">
    ...
    <div class="hidden md:block bg-white rounded-xl border border-gray-200 shadow-card overflow-hidden">
      ...
      @forelse ($users as $user)
        <div class="grid grid-cols-12 px-4 py-3 border-b border-gray-100 hover:bg-gray-50 items-center transition-colors">
          ...
          <div class="col-span-8 text-gray-800 font-medium truncate">{{ $user->name }}</div>
          <div class="col-span-3 flex justify-end">
            <div class="action-buttons">
              ...
              <form action="{{ route('users.destroy', $user->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pengguna ini?')" class="inline">
                @csrf
                @method('DELETE')
                <button type="submit"
      ...
      @empty
        <div class="empty-state px-4 py-8 text-center rounded-lg">
          ...
          <p class="text-gray-400 text-sm mt-1">Tambahkan pengguna baru dengan menekan tombol +</p>
        </div>
      @endforelse
    </div>
    ...
    <div class="md:hidden space-y-3">
      @forelse ($users as $user)
        <div class="user-card bg-white rounded-lg shadow-card p-4 transition-all">
          <div class="flex justify-between items-start">
            <div>
              <h3 class="user-title text-gray-800">{{ $user->name }}</h3>
            </div>
            <div class="action-buttons">
              <a href="{{ route('users.edit', $user->id) }}"
                 class="btn-animate action-btn bg-primary-50 text-primary-600 hover:bg-primary-100"
              ...
              <form action="{{ route('users.destroy', $user->id) }}" method="POST" onsubmit="return confirm('Hapus pengguna ini?')" class="inline">
                @csrf
                @method('DELETE')
                <button type="submit"
                        class="btn-animate action-btn bg-red-50 text-red-600 hover:bg-red-100"
